...
markers <- union(read.delim("gene lists region specific/common/astrocyte_up.txt", header = FALSE)$V1,
                 read.delim("gene lists region specific/common/astrocyte_dn.txt", header = FALSE)$V1)
...
pdf("plots/astrocyte analysis/common_astro_hmp.pdf", width = 5, height = 6)
...
#CA1 up
tmp <- read.delim("gene lists region specific/all/astrocyte_ca1_up.txt", header = FALSE)$V1
...
ggsave("plots/astrocyte analysis/ca1_up_enrichplt.svg", width = 4, height = 2.5)

#CA1 dn
tmp <- read.delim("gene lists region specific/all/astrocyte_ca1_dn.txt", header = FALSE)$V1
plt_dt <- enrichr(tmp, dbs)
plt_dt <- do.call(rbind, plt_dt)
plt_dt <- plt_dt %>% arrange(P.value) %>% select(Term, Overlap, P.value, Combined.Score) %>%
  mutate(Term = stringr::str_trunc(gsub(" \\(..*", "", Term), 40),
...
ggplot(plt_dt, aes(y=Term, x=-log10(`P.value`), color=`Combined.Score`, size=n)) +
  ...
  theme_classic() +
  theme(legend.position = c(0.8,0.8))
ggsave("plots/astrocyte analysis/ca1_dn_enrichplt.svg", width = 4, height = 2.5)

#th up
tmp <- read.delim("gene lists region specific/all/astrocyte_th_up.txt", header = FALSE)$V1
plt_dt <- enrichr(tmp, dbs)
plt_dt <- do.call(rbind, plt_dt)
plt_dt <- plt_dt %>% arrange(P.value) %>% select(Term, Overlap, P.value, Combined.Score) %>%
  mutate(Term = stringr::str_trunc(gsub(" \\(..*", "", Term), 40),
...
ggplot(plt_dt, aes(y=Term, x=-log10(`P.value`), color=`Combined.Score`, size=n)) +
  ...
  theme_classic() +
  theme(legend.position = c(0.8,0.8))
ggsave("plots/astrocyte analysis/th_up_enrichplt.svg", width = 4, height = 2.5)

#th dn
tmp <- read.delim("gene lists region specific/all/astrocyte_th_dn.txt", header = FALSE)$V1
plt_dt <- enrichr(tmp, dbs)
plt_dt <- do.call(rbind, plt_dt)
plt_dt <- plt_dt %>% arrange(P.value) %>% select(Term, Overlap, P.value, Combined.Score) %>%
  mutate(Term = stringr::str_trunc(gsub(" \\(..*", "", Term), 40),
...
ggplot(plt_dt, aes(y=Term, x=-log10(`P.value`), color=`Combined.Score`, size=n)) +
  ...
  theme_classic() +
  theme(legend.position = c(0.8,0.8))
ggsave("plots/astrocyte analysis/th_dn_enrichplt.svg", width = 4, height = 2.5)
